...
class TestArgumentParser:
    ...
    def test_parser_custom_configuration(self):
        ...
        args = parser.parse_args([
            ...
            '--species', 'H', 'He', 'H2O',
            '--top-n', '10',
            '--processing-type', 'radial_profile',
            '--timestep', '50',
            '--output-folder', 'CustomTest'
        ])
    ...
    def test_parser_file_configuration(self):
        ...
        args = parser.parse_args([
            ...
            '--reaction-file', 'custom.reac',
            '--species-file', 'custom_species.txt'
        ])
...
class TestPipelineInitialization:
    ...
    @patch('os.makedirs')
    def test_validate_configuration_missing_pumpkin_dir(self, mock_makedirs, mock_exists):
        ...
        with patch('builtins.print') as mock_print:
            pipeline = AIOLOSPumpKinPipeline(mock_args)
            mock_print.assert_called_with(f"Warning: PumpKin directory /nonexistent/pumpkin does not exist")
...
class TestAIOLOSDataProcessing:
    ...
    @patch('os.makedirs')
    def test_process_simulation_data_timesteps(self, mock_makedirs, mock_make_rates, mock_process_ts):
    ...
    @patch('making_rates_file.make_rates')
    @patch('os.makedirs')
    def test_process_simulation_data_radial_profile(self, mock_makedirs, mock_make_rates, mock_process_rad):
    ...
    def test_process_reaction_files_success(self, mock_transform, mock_process_line, 
                                          mock_parse_data, mock_exists):
        """Test successful reaction file processing."""
        mock_exists.return_value = True
        pipeline = self.create_mock_pipeline()
        
        # Mock return values
        mock_parse_data.return_value = (['reaction1'], [{'H': 1, 'O2': 1}], ['H', 'O2', 'OH', 'O'])
...
class TestCompletePipeline:
    ...
    def test_run_complete_pipeline_all_success(self):
        ...
        with patch.object(pipeline, 'process_aiolos_data', return_value=True), \
             patch.object(pipeline, 'run_pumpkin_analysis', return_value=True), \
             patch.object(pipeline, 'generate_plots', return_value=True):
    ...
    def test_run_complete_pipeline_partial_failure(self):
        ...
        with patch.object(pipeline, 'process_aiolos_data', return_value=True), \
             patch.object(pipeline, 'run_pumpkin_analysis', return_value=False), \
             patch.object(pipeline, 'generate_plots', return_value=True):
...
class TestMainFunction:
    ...
    def test_main_no_mode_selected(self):
        ...
        assert result == 1
        mock_print.assert_called_with("No operation mode selected. Use --help for options or --run-all for complete pipeline.")
...
class TestIntegratedMainWorkflow:
    ...
    @patch('os.makedirs')
    @patch('subprocess.run')
    def test_complete_workflow_simulation(self, mock_subprocess, mock_makedirs, mock_exists):
        ...
        with patch('processing_aiolos_reac_file.parse_reaction_data') as mock_parse, \
             patch('processing_aiolos_reac_file.process_reaction_line') as mock_process, \
             patch('processing_aiolos_reac_file.transform_species_set') as mock_transform, \
             patch('making_densities_file.process_timesteps') as mock_process_ts, \
             patch('making_rates_file.make_rates') as mock_make_rates:
