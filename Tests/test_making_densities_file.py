...
from making_densities_file import get_essential_data, process_timesteps, process_radial_profile
...
class TestGetEssentialData:
    ...
    def create_mock_data_file(self, n_rows=50):
        ...
        data = np.column_stack([
            radius, col1, col2, col3, number_density, col5, col6, col7,
            col8, col9, col10, temperature, col12
        ])
        return data
    ...
    def test_species_data_arrays(self, mock_isfile, mock_genfromtxt):
        ...
        avg_T, r, all_data = get_essential_data("data/", "sim1", 10, species, rplanet=1.37e8)
    ...
    def test_planet_radius_normalization(self, mock_isfile, mock_genfromtxt):
        ...
        avg_T, r, all_data = get_essential_data("data/", "sim1", 10, species, rplanet=-1)
        assert r[0] == mock_data[0, 0] / 1.5e8  # Should normalize by auto-detected radius
        
        # Test manual planet radius
        avg_T, r, all_data = get_essential_data("data/", "sim1", 10, species, rplanet=2e8)
...
class TestProcessTimesteps:
    ...
    def test_basic_timestep_processing(self, mock_get_data):
        ...
        mock_species_data = [
            np.array([[1e8, 0, 1e12, 0, 100], [1.5e8, 0, 5e11, 0, 200], [2e8, 0, 1e11, 0, 300], 
                     [2.5e8, 0, 5e10, 0, 400], [3e8, 0, 1e10, 0, 500]]),  # H data
            np.array([[1e8, 0, 5e11, 0, 100], [1.5e8, 0, 2e11, 0, 200], [2e8, 0, 5e10, 0, 300],
                     [2.5e8, 0, 2e10, 0, 400], [3e8, 0, 5e9, 0, 500]])   # He data
        ]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            avg_T_list, data_dict = process_timesteps(
                "data/", "sim1", timesteps, species,
                index=2, output_file=output_file, output_file_2=output_file_2
            )
    ...
    def test_timestep_processing_with_errors(self, mock_get_data):
        ...
        def side_effect(*args, **kwargs):
            if kwargs.get('timestep') == 5:
                ...
                mock_species_data = [np.array([[1e8, 0, 1e12, 0, 100], [1.5e8, 0, 5e11, 0, 200], [2e8, 0, 1e11, 0, 300]])]
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            avg_T_list, data_dict = process_timesteps(
                "data/", "sim1", timesteps, species,
                output_file=output_file, output_file_2=output_file_2
            )
    ...
    def test_file_creation_error(self, mock_get_data):
        ...
        with pytest.raises(IOError):
            process_radial_profile(
                "data/", "sim1", 85, species,
                output_file=invalid_path, output_file_2=invalid_path
            )
    ...
    def test_timesteps_to_radial_consistency(self, mock_get_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            avg_T_ts, data_ts = process_timesteps(
                "data/", "sim1", [timestep], species,
                index=index, output_file=ts_file1, output_file_2=ts_file2
            )
            ...
            avg_T_rad, data_rad = process_radial_profile(
                "data/", "sim1", timestep, species,
                output_file=rad_file1, output_file_2=rad_file2
            )
