...
class TestProcessSide:
    ...
    def test_complex_species_names(self):
        ...
        # The regex pattern captures "H3O" not "H3O+" because + is not included \n        # in the species pattern
...
class TestParseReactionData:
    ...
    def test_malformed_reactions(self):
        ...
        # The function currently parses all reaction lines but only creates 
        # stoichiometry for valid ones
        assert len(reactions) == 3  # All lines are parsed as reactions
        assert (
            "1 H2 + 1 O2 -> 1 H2O" in reactions[1]
        )  # The good reaction is the second one
...
class TestIntegratedReactionProcessing:
    ...
    def test_complete_aiolos_workflow(self):
        ...
        for processed in processed_reactions:
            ...
            # Check that stoichiometric coefficients were removed 
            # (no standalone numbers)
            reactants = processed.split("=>")[0]
            # Should not have patterns like "2 H" or "1.5 O2" - numbers should 
            # only be part of species names
            import re
        ...
        for stoich in stoich_list:
            ...
            if (
                "M" not in stoich and len(stoich) > 1
            ):  # Skip single-species reactions (likely photodissociation)
