...
This script provides a unified interface to run all plotting functions in the Outputs folder:
...
try:
    from spagehtti_plots import (
        plot_densities_and_mixing_ratios,
        main as spaghetti_main
    )
...
def run_species_plots(args):
    ...
    for species in args.species:
        ...
        if args.plot_top_pathways:
            plot_top_species_pathways_by_rate(
                cell_dict, species, top_n=args.top_n,
        ...
        if args.plot_stacked:
            plot_stacked_production_fraction(
                cell_dict, species, top_n=args.top_n,
                save_path=os.path.join(args.output_dir, f"{species}_stacked_fraction.png")
            )
        
        # Net percentage vs temperature
        if args.plot_net_pct_temp and args.temp_file:
            plot_species_net_percentage_vs_temperature(
                cell_dict, args.temp_file, species,
                save_path=os.path.join(args.output_dir, f"{species}_net_pct_vs_temp.png")
            )
    ...
    if args.plot_pathway_matrix:
        ...
        plot_pathway_species_heatmap(
            matrix, save_path=os.path.join(args.output_dir, "pathway_species_matrix.png")
        )
...
def run_spaghetti_plots(args):
    ...
    try:
        spaghetti_main(
            ...
            output_plot=os.path.join(args.output_dir, "species_density_and_mixing_ratios.png")
        )
...
Examples:
  ...
  python plotting_main.py --spaghetti --species-file ../qt_species_list.txt --densities-file ../qt_densities.txt
    ...
    parser.add_argument('--species-plots', action='store_true', help='Run species-specific plots')
    parser.add_argument('--pathway-rates', action='store_true', help='Run pathway rates plots')
    parser.add_argument('--reaction-rates', action='store_true', help='Run reaction rates plots')
    parser.add_argument('--deleted-pathways', action='store_true', help='Run deleted pathways plots')
    parser.add_argument('--spaghetti', action='store_true', help='Run spaghetti plots (densities/mixing ratios)')
    
    # Common parameters
    parser.add_argument('--data-dir', type=str, default='./', help='Directory containing data files (default: ./)')
    parser.add_argument('--output-dir', type=str, default='./Plots/', help='Output directory for plots (default: ./Plots/)')
    parser.add_argument('--num-cells', type=int, default=233, help='Number of cells to process (default: 233)')
    parser.add_argument('--file-pattern', type=str, default='pumpkin_output_cell_*.txt', help='Pattern for cell files (default: pumpkin_output_cell_*.txt)')
    
    # Species-specific parameters
    parser.add_argument('--species', nargs='+', default=['H', 'H2', 'O', 'OH'], help='Species to plot (default: H H2 O OH)')
    parser.add_argument('--top-n', type=int, default=5, help='Number of top pathways to show (default: 5)')
    parser.add_argument('--temp-file', type=str, default='qt_conditions.txt', help='Temperature conditions file (default: qt_conditions.txt)')
    
    # Species plot type controls
    parser.add_argument('--plot-top-pathways', action='store_true', default=True, help='Plot top pathways (default: True)')
    parser.add_argument('--plot-net-rate', action='store_true', default=True, help='Plot net rates (default: True)')
    parser.add_argument('--plot-heatmap', action='store_true', default=True, help='Plot pathway heatmaps (default: True)')
    parser.add_argument('--plot-stacked', action='store_true', default=True, help='Plot stacked production fractions (default: True)')
    parser.add_argument('--plot-net-pct-temp', action='store_true', default=True, help='Plot net percentage vs temperature (default: True)')
    parser.add_argument('--plot-pathway-matrix', action='store_true', default=True, help='Plot pathway-species matrix (default: True)')
    
    # Pathway rates parameters
    parser.add_argument('--use-multi-line-parser', action='store_true', default=True, help='Use multi-line pathway parser (default: True)')
    parser.add_argument('--top-n-pathways', type=int, default=25, help='Number of top pathways for pathway rates plot (default: 25)')
    
    # Reaction rates parameters
    parser.add_argument('--reactions-file', type=str, default='qt_reactions_list.txt', help='Reactions list file (default: qt_reactions_list.txt)')
    parser.add_argument('--rates-file', type=str, default='qt_rates.txt', help='Rates file (default: qt_rates.txt)')
    
    # Spaghetti plot parameters
    parser.add_argument('--species-file', type=str, default='qt_species_list.txt', help='Species list file (default: qt_species_list.txt)')
    parser.add_argument('--densities-file', type=str, default='qt_densities.txt', help='Densities file (default: qt_densities.txt)')
...
def main():
    ...
    if not any([args.all, args.species_plots, args.pathway_rates, args.reaction_rates, args.deleted_pathways, args.spaghetti]):
        print("No plotting mode selected. Use --help for options or --all to run everything.")
        return 1
