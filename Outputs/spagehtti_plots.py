    ...
    try:
        ...
        for i, sp in enumerate(species):
            if np.any(densities[:, i] > 0):
                ax1.plot(cells, densities[:, i], label=sp)
        ax1.set_xscale('log')
        ax1.set_yscale('log')
        ax1.set_xlabel('Cell Number')
        ax1.set_ylabel('Number Density')
        ax1.set_title('Species Number Density vs. Cell Number')
        ...
        for i, sp in enumerate(species):
            if np.any(mixing_ratios[:, i] > 0):
                ax2.plot(cells, mixing_ratios[:, i], label=sp)
        ax2.set_xscale('log')
        ax2.set_yscale('log')
        ax2.set_xlabel('Cell Number')
        ax2.set_ylabel('Mixing Ratio')
        ax2.set_title('Species Mixing Ratio vs. Cell Number')
