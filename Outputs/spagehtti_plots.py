...
def plot_densities_and_mixing_ratios(
    species_file, densities_file, save_path="species_density_and_mixing_ratios.png"
):
    """
    Reads species names and densities, plots densities and mixing ratios vs. 
    cell number on a log-log scale,
    and saves the plot.

    Args:
        species_file (str): Path to file with species names (one per line).
        densities_file (str): Path to file with densities. First col: cell, 
            rest: densities.
    ...
    try:
        ...
        density_cols = [
            col for col in density_df.columns if col.startswith("Density_Sp")
        ]
        if len(density_cols) != len(species):
            print(
                f"Error: Number of density columns ({len(density_cols)}) "
                f"does not match species list ({len(species)})."
            )
            return
...
def main(
    ...
    plot_densities_and_mixing_ratios(
        species_file, densities_file, save_path=output_plot
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Plot densities and mixing ratios, save to PNG."
    )
    parser.add_argument(
        "--species",
        type=str,
    ...
    parser.add_argument(
        "--densities", type=str, default="../qt_densities.txt", help="Densities file"
    )
    parser.add_argument(
        "--out",
        type=str,
        default="./species_density_and_mixing_ratios.png",
