...
from Outputs.deleted_pathways_plots import (
    parse_deleted_percent_block,
    parse_file,
...
def test_parse_deleted_percent_block_basic():
    ...
    lines = ["|   H2O   |   15.0%   |   3.2%   |", "|   CO2   |   7.1%    |   12.0%  |"]
...
"""
    ...
    valid_species = [
        k
        for k, v in deleted_contributions.items()
        if not (
            all(np.isnan(x) for x in v["production"])
            and all(np.isnan(x) for x in v["consumption"])
        )
    ]
