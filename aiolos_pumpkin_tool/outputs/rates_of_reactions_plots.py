...
def plot_reaction_rates(reactions_file, rates_file, save_path="reaction_rates.png"):
    """
    Reads reaction names and rates, plots reaction rates vs cell number, and 
    saves the plot.
    ...
    try:
        ...
        try:
            rate_df = pd.read_csv(
                rates_file, sep="\s+", header=0, dtype=float, engine="python"
            )
            print(
                f"Successfully read rates file using read_csv with header=0. "
                f"Shape: {rate_df.shape}"
            )

        except Exception as e:
            print(f"Error reading rates file with pandas: {e}")
            print(
                "Please check the file format, especially the header and the "
                "data columns."
            )
            traceback.print_exc()
            return
        ...
        if rate_df.shape[1] == len(reactions) + 1:
            ...
            try:
                ...
                print(
                    "Error: Cannot determine cell numbers. Rates file should have "
                    "cell numbers in header or first column."
                )
                return
        else:
            print(
                f"Error: Unexpected number of columns in rates file "
                f"({rate_df.shape[1]}). Expected {len(reactions)} or "
                f"{len(reactions)+1}."
            )
            return
        ...
        if rates.shape[0] != len(reactions):
            print(
                f"Error: Number of transposed rates rows ({rates.shape[0]}) "
                f"does not match the number of reactions ({len(reactions)})."
            )
            return
        ...
        reactions_to_plot_indices = [
            i for i in range(rates.shape[0]) if np.any(rates[i, :] > 0)
        ]
        ...
        for i in reactions_to_plot_indices:
            reaction_name = reactions[i]
            reaction_rates_across_cells = rates[i, :]
            rates_filtered = np.where(
                reaction_rates_across_cells > zoom_threshold,
                reaction_rates_across_cells,
                np.nan,
            )
        ...
        ax_zoomed.set_title(
            f"Reaction Rate vs. Cell Number (Rates > {zoom_threshold:.0e})",
            fontweight="bold",
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="Path to reactions list",
    )
    parser.add_argument(
        "--rates", type=str, default="../qt_rates.txt", help="Path to rates file"
    )
    parser.add_argument(
        "--out", type=str, default="reaction_rates.png", help="Output plot image file"
    )
    args = parser.parse_args()

    main(reactions_path=args.reactions, rates_path=args.rates, output_plot=args.out)
